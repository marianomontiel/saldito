...
function settleUp() {
    ...
    bills = Array.from(document.querySelectorAll('.bill')).map(bill => {
        const name = bill.querySelector('.billName').value,
            ...
            billParticipants = bill.querySelector('.billParticipants').value.split(',').map(name => name.trim()),
            payer = bill.querySelector('.payer');
        return { name, amount, billParticipants, payer };
    });
